...
`EPHEMERAL-STEGBROWSER-EXTERNAL-AI-ACTIVATION-001` uses the existing `ProviderRequest` / `ProviderResponse` and `external_llm_connection` convergence seam. `llm_adapter/openai_tvc_runtime_executor.py` is a narrow OpenAI Responses edge that accepts an exact independently admitted InTr request and a current task-specific TVC single-use lease, submits the exact prompt/model through TVC's non-exportable local vault broker, validates provider result and native usage, requires an existing canonical organization-receipt callback and independently required usage custody, and returns an exact response for separate InTr egress authorization. It never consumes a measurement-purpose TVC lease or opens process-environment provider keys. StegBrowser, WorkerCoordinator, TVC, InTr, organization-ledger and conditional Master Records custody remain separately owned. The browser terminates disposable sessions separately from the provider adapter. Native MyKV installation is not required for non-private inputs. See `docs/EPHEMERAL_STEGBROWSER_OPENAI_TVC_MIRROR_HANDOFF.md`. Source and CI validation do **not** prove actual resident execution or benchmark completion.
