...
## Task-bound ephemeral OpenAI through existing TVC

`EPHEMERAL-STEGBROWSER-EXTERNAL-AI-ACTIVATION-001` uses the existing `ProviderRequest` / `ProviderResponse` and `external_llm_connection` convergence seam. `llm_adapter/openai_tvc_runtime_executor.py` is a narrow OpenAI Responses edge that accepts an exact independently admitted InTr request and a current task-specific TVC single-use lease, submits the exact prompt/model through TVC's non-exportable local vault broker, validates provider result and native usage, requires an existing canonical organization-receipt callback and independently required usage custody, and returns an exact response for separate InTr egress authorization. The caller must also supply an existing-resident read-only `current_admission_verifier` that authenticates the exact TVC-issued receipt and currently active WorkerCoordinator claim/fence and StegBrowser lease; ordinary self-hashed lease JSON cannot authorize a provider call. It never consumes a measurement-purpose TVC lease or opens process-environment provider keys. StegBrowser, WorkerCoordinator, TVC, InTr, organization-ledger and conditional Master Records custody remain separately owned. The browser terminates disposable sessions separately from the provider adapter. Native MyKV installation is not required for non-private inputs. See `docs/EPHEMERAL_STEGBROWSER_OPENAI_TVC_MIRROR_HANDOFF.md`. Source and CI validation do **not** prove actual resident execution or benchmark completion.
...
## Ecosystem Chat provider contribution binding

The governed external ProviderClient returns the exact caller-declared provider alias and original ProviderRequest envelope hash expected by the existing distributed Chat executor. Before egress it checks the canonical provider, model, wire request digest and provider response commitment. Returned metadata retains the distinct `wire_request_hash`, `admitted_provider_response_hash`, ingress/egress receipt hashes and measured usage event references. The projected response hash is not mislabeled as the provider response approved by InTr. This fixes composition for the existing provider families, including the proposed OpenAI path; it grants no transition permission and proves no live provider or ephemeral-browser execution.
