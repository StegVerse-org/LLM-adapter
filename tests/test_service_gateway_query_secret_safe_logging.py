...
from llm_adapter.query_secret_safe_access_log import QuerySecretSafeAccessLogMiddleware
...
def test_oauth_callback_query_never_enters_access_log():
    secret = b"code=oauth-auth-code-123&state=owner-state-456&other=private-value"
    value = asyncio.run(_exercise(secret))
...
def test_safe_middleware_never_reads_query_or_sensitive_request_surfaces():
    source = (Path(__file__).resolve().parents[1] / "llm_adapter" / "query_secret_safe_access_log.py").read_text(encoding="utf-8")
    for prohibited in (
