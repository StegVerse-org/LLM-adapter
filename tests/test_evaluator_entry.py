...
class EvaluatorEntryTests(unittest.TestCase):
    ...
    def test_execution_receipt_is_bounded_and_non_authorizing(self):
        rel = relationship(); req = request(rel)
        receipt = execute_evaluator_entry(request=req, relationship=rel, local_reference_executor=lambda p, n: {"text":"bounded answer","model_id":"stegverse-reference-lm-v1","measured_usage":{"prompt_tokens":5,"completion_tokens":2,"total_tokens":7}})
