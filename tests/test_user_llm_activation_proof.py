...
def test_activation_proof_defers_without_complete_endpoint_set():
    proof = build_activation_proof(HTTPRouteConfig(demo_test_suite_url="https://demo.example/submit"))
...
def test_activation_proof_is_deterministic_and_hides_endpoint_values():
    ...
    assert first["routes"][0]["endpoint_hash"] == sha256(b"https://demo.example/submit").hexdigest()
