...
credential/provider-operation authority; provider output remains non-authoritative.
"""
...
def execute_kimi_via_tvc_broker(
    ...
    *,
    ...
    if not isinstance(output, str) or not output.strip():
        output = result.get("candidate_output")
    if not isinstance(output, str) or not output.strip():
        raise KimiTVCBrokerError("TVC Kimi result missing text output")
