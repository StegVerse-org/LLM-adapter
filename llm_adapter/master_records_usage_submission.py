...
def _configured() -> MasterRecordsUsageConfig | None:
    ...
    if parsed.scheme != "https" and not (local_http and allow_local):
...
def submit_provider_usage_to_master_records(
    event: dict[str, Any],
    *,
    opener: Callable[..., Any] = request.urlopen,
) -> dict[str, Any]:
    """Submit one canonical provider usage event and validate the custody receipt.

    Missing configuration is a visible non-error state. Partial or unsafe
    configuration fails closed. The token is used only in the request header.
    """
    config = _configured()
    if config is None:
        return {
    ...
    body = json.dumps(
        {
            "schema": "stegverse.master_records.provider_usage_submission.v1",
            "event": event,
    ...
    outbound = request.Request(
        ...
        headers={
            ...
            "X-SteGVerse-Session": str(event["session_id"]),
        },
    )
    try:
    ...
    return {
        ...
        "reconstructability": validated.get("reconstructability", "PENDING"),
        "authority_granted": False,
