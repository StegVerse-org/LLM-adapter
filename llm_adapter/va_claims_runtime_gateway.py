...
from .sovereign_local_model_binding import SovereignLocalModelBindingError, execute_verified_local_model
...
def _load_object_env(name: str) -> dict[str, Any]:
...
def _validate_tvc_route(route: dict[str, Any], proof: dict[str, Any]) -> str:
    ...
    if route.get("runtime_proof_hash") != _stable_hash(proof):
...
def execute_chat(request: ChatRequest) -> dict[str, Any]:
    ...
    registry = _load_object_env("STEGVERSE_VA_SOURCE_REGISTRY_FILE")
    proof = _load_object_env("STEGVERSE_CANONICAL_RUNTIME_PROOF_FILE")
    route_receipt = _load_object_env("STEGVERSE_TVC_ROUTE_RECEIPT_FILE")
    ...
    transport_endpoint = _validate_tvc_route(route_receipt, proof)
    ...
    response = {
        ...
        "execution_receipt_hash": receipt.get("receipt_hash") or _stable_hash(receipt),
...
def readiness() -> dict[str, Any]:
    try:
        ...
        proof = _load_object_env("STEGVERSE_CANONICAL_RUNTIME_PROOF_FILE")
        route_receipt = _load_object_env("STEGVERSE_TVC_ROUTE_RECEIPT_FILE")
        _validate_tvc_route(route_receipt, proof)
