...
def execute_governed_external_llm(
    ...
    **provider_options: Any,
    ...
    base = dict(request=request, session_id=session_id, transition_id=transition_id,
        measurement_id=measurement_id, ingress_disposition=ingress_disposition,
        ingress_receipt_hash=ingress_receipt_hash, carrier_ref=carrier_ref)

    if provider in {"deepseek", "kimi"} and lease_receipt is not None and broker_submitter is not None:
        tvc = dict(base, lease_receipt=lease_receipt, broker_submitter=broker_submitter,
        ...
        if provider == "deepseek":
            execution = execute_governed_deepseek_via_tvc_runtime(**tvc)
        else:
            execution = execute_governed_kimi_via_tvc_runtime(**tvc)
    ...
    if not callable(credential_resolver):
        raise ExternalLLMConnectionError(
            f"{provider} requires TV/TVC execution material: supply canonical TVC lease+broker when supported, otherwise a TV/TVC credential resolver"
        )
    direct = dict(base, credential_resolver=credential_resolver)
    if provider == "zai":
        execution = execute_governed_zai(**direct, endpoint_profile=provider_options.get("endpoint_profile", "general"))
    elif provider == "deepseek":
        execution = execute_governed_deepseek(**direct)
    elif provider == "kimi":
        execution = execute_governed_kimi(**direct)
    elif provider == "anthropic":
        execution = execute_governed_anthropic(**direct, max_tokens=int(provider_options.get("max_tokens", 1024)))
    else:
        raise ExternalLLMConnectionError("provider dispatch invariant violated")
...
def admit_external_llm_egress(result: GovernedConnectionResult, *, egress_disposition: str, egress_receipt_hash: str, admitted_response_hash: str) -> Any:
    kwargs = dict(execution=result.execution, egress_disposition=egress_disposition,
        egress_receipt_hash=egress_receipt_hash, admitted_response_hash=admitted_response_hash)
